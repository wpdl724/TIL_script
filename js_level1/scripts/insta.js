...
//(아래) likeImg객체가 a태그로 클릭시 새로고침되서 스크롤이 위로 올라가는 문제 해결을 위한방법
//콜백함수 호출이 아닌 익명함수 또는 화살표함수 이용필수
//익명함수 또는 화살표함수로 작성 시 이벤트 앞 객체정보가 함수의 매개변수 정보로 자동으로 대입된다.
// 위 매개변수가 받은 정보를 확인하려면 매개변수에 변수명(e)이벤트의 약자 을 작성하고 console.log()로 확인 할수 있다
likeImg.addEventListener('click',function(e){
    console.log(e);
    e.preventDefault(); // 태그의 동적기능 취소시킨다(a태그 새로고침)
    likeOn();
});
